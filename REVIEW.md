...
| 54 | `admin.html` | **Mất trắng danh sách Shop chưa lưu.** `removeShop()` / `importShopCSV()` sửa `_shopList` nhưng không đặt cờ nào; cả `beforeunload` lẫn chốt chặn auto-redirect khi idle đều chỉ kiểm tra `dirty` (chỉ theo dõi playlist). Hệ quả: import CSV vài trăm shop rồi rời máy 2 phút → trang **tự chuyển sang URL display, mất sạch, không một lời cảnh báo**. Cùng loại lỗi đã fix ở #17 cho playlist. Kiểm chứng: sau `removeShop(0)` thì `dirty` vẫn `false` ⇒ cả 2 lớp bảo vệ đều không chặn | Thêm cờ riêng `_shopsDirty` + `markShopsDirty()`; đặt khi xoá/import, xoá khi lưu xong hoặc vừa tải từ GitHub; cả 2 lớp bảo vệ đổi thành `dirty \|\| _shopsDirty`. Thêm chỉ báo "● chưa lưu" cạnh tiêu đề khối Shop và nhắc trong toast sau khi import |
...
| # | File | Feature |
|---|------|---------|
| F3 | `admin.html`, `functions/api/shops.js`, `shops.json`, `config-watcher.js` | **Danh sách Shop + báo cáo mở rộng qua heartbeat.** `shops.json` (danh sách `{name, region}`) quản lý trong admin: upload CSV, xoá từng dòng, lưu lên GitHub qua `/api/shops` (cùng pattern `/api/config`). Máy Android tải **thẳng file tĩnh** `shops.json` (không qua route API) để người lắp máy chọn Shop lúc kích hoạt; shop chọn xong đi vào URL dưới dạng `?shop=`. Heartbeat mở rộng thêm `shopName`, `appVersion`, `deviceModel` (bóc từ User-Agent do app Android chèn: `CPSDisplayApp/1.0.6`, `CPSDisplayModel/...`) và `lastError` (chỉ báo nếu lỗi xảy ra trong 1 giờ gần đây — tránh báo lỗi cũ đã tự khỏi). Bảng máy trong admin hiện đủ các cột này + bộ lọc theo shop. Các khối phụ (Shop / Lịch sử / Máy) mặc định thu gọn, nhớ trạng thái qua `localStorage`. **Lưu ý:** `shops.json` nằm ở thư mục gốc nên **truy cập công khai được** — danh sách mã cửa hàng không bí mật, nhưng đừng đưa dữ liệu nhạy cảm vào đây. |
...
| 50 | `admin.html` | **Campaign "mồ côi" — UI nói dối admin.** Xoá 1 chương trình trong khi vẫn còn video gắn nó thì `removeCampaign()` chỉ xoá khỏi `campaignList`, **không gỡ tag khỏi video**. Card khi đó không có `<option>` nào khớp → trình duyệt chọn option đầu → hiển thị **"— Luôn chạy"**, trong khi `buildConfigJs()` vẫn ghi `campaign: 'trade'`. Kết quả: video chỉ phát trên thiết bị có `?c=trade` nhưng tên đó không còn trong UI để admin lần ra. Kiểm chứng: `{dataCampaign:'trade', uiHienThi:'— Luôn chạy', configGhiRa:["campaign: 'trade'"], appConfigCampaigns:"campaigns: ['scamp']"}`. **Đường vào thứ 2:** `importCSV()` gán `entry.campaign` nhưng không đăng ký vào `campaignList` → sai lệch y hệt | (1) `removeCampaign()` gỡ tag khỏi mọi video + `confirm()` nêu rõ số video bị ảnh hưởng, bấm Huỷ thì không đổi gì. (2) `importCSV()` thêm campaign mới vào `campaignList` + refresh chips. (3) Lưới an toàn ở `loadFromGitHub()`: `campaignList = union(APP_CONFIG.campaigns, campaign đang được video dùng)` — tự chữa các config đã lỡ có orphan từ trước |
